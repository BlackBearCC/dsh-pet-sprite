...
*/
function contextPrompt(lang, workspace, memories) {
	const zh = lang.toLowerCase().startsWith("zh");
	const parts = [];
	...
	if (webServer !== void 0 && llm !== void 0) ctx.effect(() => webServer.register({
		...
		handler: async (rawReq, rawRes) => {
			...
			try {
				...
				const system = personaPrompt(petName, body.persona ?? "", body.lang ?? "zh") + contextPrompt(body.lang ?? "zh", body.workspace, body.memories);
				const options = {
					provider,
					model,
					system,
		...
		handler: async (rawReq, rawRes) => {
			...
			try {
				...
				const options = {
					provider,
					model,
					system: personaPrompt(petName, body.persona ?? "", lang) + contextPrompt(lang, void 0, body.memories),
					messages: [{
						id: crypto.randomUUID(),
						role: "user",
						content: [{
	...
	if (webServer !== void 0 && llm !== void 0) ctx.effect(() => webServer.register({
		...
		handler: async (rawReq, rawRes) => {
			...
			try {
				...
				const petName = (body.petName ?? "").trim() || "小宠物";
				const lang = body.lang ?? "zh";
				const existing = (body.existing ?? []).filter((t) => typeof t === "string").map((t) => t.trim().slice(0, 60)).filter((t) => t.length > 0).slice(0, 30);
				const options = {
					provider,
					model,
