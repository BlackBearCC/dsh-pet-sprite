...
const CarePanel = ({ engine, anchor, petName, chatModel, onChatModelChange, persona, onPersonaChange, onGeneratePet, onImportPet, onSwitchPet, onClose }) => {
...
function downloadShareFile(pet) {
...
const PetCard = ({ pet, custom, isCurrent, state, onPick }) => {
...
const PetPicker = ({ currentId, customPets, onPick, onClose }) => {
...
const ChatPet = () => {
	...
	const [persona, setPersona] = (0, react.useState)(() => loadPersona());
			...
			if (now > platsAt) {
				...
				if (!playerCtl && !dragging && mode === "idle" && state === "ground") {
					if (nextChatAt === 0) nextChatAt = now + 15e3 + Math.random() * 15e3;
						...
						const s = engine.getStats();
						say(s.power < 30 || s.mood < 30 ? pick(CHAT_LINES.low) : pick([...CHAT_LINES.idle, ...idleLines]));
